...
test("should sync multiple sessions in a single content message", async () => {
    ...
    expect(toSimplifiedMessages({
        Map: map.core,
        Group: group.core,
    }, node3Sync.messages)).toMatchInlineSnapshot(`
    [
      "client -> LOAD Map sessions: empty",
      "storage -> CONTENT Group header: true new: After: 0 New: 3",
      "client -> KNOWN Group sessions: header/3",
...
test("large coValue upload streaming", async () => {
    ...
    await waitFor(() => {
        expect(largeMapOnNode2.core.knownState()).toEqual(largeMap.core.knownState());
        return true;
    });
    expect(toSimplifiedMessages({
        Map: largeMap.core,
...
//# sourceMappingURL=storage.sqliteAsync.test.js.map