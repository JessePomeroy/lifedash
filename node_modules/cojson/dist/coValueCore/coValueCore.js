...
import { getParentGroupId, isParentGroupReference, } from "../ids.js";
...
export class CoValueCore {
    ...
    provideHeader(header, fromPeerId) {
        const previousState = this.loadingState;
    ...
    async loadFromPeers(peers) {
        ...
        if (peersToActuallyLoadFrom.storage.length > 0) {
            await Promise.all(peersToActuallyLoadFrom.storage.map((peer) => this.internalLoadFromPeer(peer)));
        }
        if (peersToActuallyLoadFrom.server.length > 0) {
            await Promise.all(peersToActuallyLoadFrom.server.map((peer) => this.internalLoadFromPeer(peer)));
        }
    }
