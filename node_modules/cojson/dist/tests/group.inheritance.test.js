...
import { createThreeConnectedNodes, createTwoConnectedNodes, loadCoValueOrFail, } from "./testUtils";
describe("extend", () => {
...
describe("unextend", () => {
    ...
    test("should work when the account has no access to the parent group and not owns the writeKey", async () => {
        const { node1: bobNode, node2: johnNode, node3: aliceNode, } = await createThreeConnectedNodes("server", "server", "server");
        const parentGroup = bobNode.node.createGroup();
        const childGroup = johnNode.node.createGroup();
        const parentGroupOnJohn = await loadCoValueOrFail(johnNode.node, parentGroup.id);
        ...
        const childGroupOnAlice = await loadCoValueOrFail(aliceNode.node, childGroup.id);
        // `childGroup` no longer has `parentGroup`'s members
...
//# sourceMappingURL=group.inheritance.test.js.map