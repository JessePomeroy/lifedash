...
import { createTestMetricReader, createTestNode, createTwoConnectedNodes, loadCoValueOrFail, nodeWithRandomAgentAndSessionID, randomAgentAndSessionID, tearDownTestMetricReader, } from "./testUtils.js";
...
test("Can create coValue with new agent credentials and add transaction to it", () => {
    ...
    const { expectedNewHash } = coValue.verified.expectedNewHashAfter(node.currentSessionID, [transaction]);
    expect(coValue
        .tryAddTransactions(node.currentSessionID, [transaction], expectedNewHash, Crypto.sign(agent.currentSignerSecret(), expectedNewHash), "immediate")
...
test("transactions with wrong signature are rejected", () => {
    ...
    coValue
        .tryAddTransactions(node.currentSessionID, [transaction], expectedNewHash, Crypto.sign(Crypto.getAgentSignerSecret(wrongAgent), expectedNewHash), "immediate")
...
test("transactions with correctly signed, but wrong hash are rejected", () => {
    ...
    const { expectedNewHash } = coValue.verified.expectedNewHashAfter(node.currentSessionID, [
        {
            privacy: "trusting",
            madeAt: Date.now(),
            changes: stableStringify([
...
test("New transactions in a group correctly update owned values, including subscriptions", async () => {
    ...
    const { expectedNewHash } = group.core.verified.expectedNewHashAfter(sessionID, [resignationThatWeJustLearnedAbout]);
    const signature = Crypto.sign(node.getCurrentAgent().currentSignerSecret(), expectedNewHash);
    expect(map.core.getValidSortedTransactions().length).toBe(1);
    const manuallyAdddedTxSuccess = group.core
        .tryAddTransactions(node.currentSessionID, [resignationThatWeJustLearnedAbout], expectedNewHash, signature, "immediate")
