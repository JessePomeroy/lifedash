...
import { SyncMessagesLog, loadCoValueOrFail, setupTestAccount, setupTestNode, } from "./testUtils.js";
...
describe("Group.removeMember", () => {
    test("a reader member should be able to revoke themselves", async () => {
        ...
        const readerOnAdminNode = await loadCoValueOrFail(admin.node, reader.accountID);
        ...
        await groupOnReaderNode.removeMember(reader.node.expectCurrentAccount("reader"));
    ...
    test("a writer member should be able to revoke themselves", async () => {
        ...
        const group = admin.node.createGroup();
        const writerOnAdminNode = await loadCoValueOrFail(admin.node, writer.accountID);
        ...
        await groupOnWriterNode.removeMember(writer.node.expectCurrentAccount("writer"));
    ...
    test("a writeOnly member should be able to revoke themselves", async () => {
        ...
        const group = admin.node.createGroup();
        const writeOnlyOnAdminNode = await loadCoValueOrFail(admin.node, writeOnly.accountID);
        group.addMember(writeOnlyOnAdminNode, "writeOnly");
        const groupOnWriteOnlyNode = await loadCoValueOrFail(writeOnly.node, group.id);
        expect(groupOnWriteOnlyNode.myRole()).toEqual("writeOnly");
        await groupOnWriteOnlyNode.removeMember(writeOnly.node.expectCurrentAccount("writeOnly"));
    ...
    test("an admin member should be able to revoke themselves", async () => {
        ...
        const group = admin.node.createGroup();
        const otherAdminOnAdminNode = await loadCoValueOrFail(admin.node, otherAdmin.accountID);
        group.addMember(otherAdminOnAdminNode, "admin");
        const groupOnOtherAdminNode = await loadCoValueOrFail(otherAdmin.node, group.id);
        expect(groupOnOtherAdminNode.myRole()).toEqual("admin");
        await groupOnOtherAdminNode.removeMember(otherAdmin.node.expectCurrentAccount("admin"));
    ...
    test("a writer member cannot remove other accounts", async () => {
        ...
        const group = admin.node.createGroup();
        const writerOnAdminNode = await loadCoValueOrFail(admin.node, writer.accountID);
        const otherMemberOnAdminNode = await loadCoValueOrFail(admin.node, otherMember.accountID);
        group.addMember(writerOnAdminNode, "writer");
        group.addMember(otherMemberOnAdminNode, "reader");
        const groupOnWriterNode = await loadCoValueOrFail(writer.node, group.id);
        expect(groupOnWriterNode.myRole()).toEqual("writer");
        const otherMemberOnWriterNode = await loadCoValueOrFail(writer.node, otherMember.accountID);
    ...
    test("a writeOnly member cannot remove other accounts", async () => {
        ...
        const groupOnWriteOnlyNode = await loadCoValueOrFail(writeOnly.node, group.id);
        expect(groupOnWriteOnlyNode.myRole()).toEqual("writeOnly");
        const otherMemberOnWriteOnlyNode = await loadCoValueOrFail(writeOnly.node, otherMember.accountID);
        await groupOnWriteOnlyNode.removeMember(otherMemberOnWriteOnlyNode);
        expect(groupOnWriteOnlyNode.roleOf(otherMember.accountID)).toEqual("reader");
    });
    test("a reader member cannot remove other accounts", async () => {
        ...
        const groupOnReaderNode = await loadCoValueOrFail(reader.node, group.id);
        expect(groupOnReaderNode.myRole()).toEqual("reader");
        const otherMemberOnReaderNode = await loadCoValueOrFail(reader.node, otherMember.accountID);
    ...
    test("removing a member when inheriting a group where the user lacks read rights", async () => {
        ...
        const childAdminOnAdminNode = await loadCoValueOrFail(admin.node, childAdmin.accountID);
        const readerOnAdminNode = await loadCoValueOrFail(admin.node, reader.accountID);
        const group = admin.node.createGroup();
        ...
        const readerOnChildAdminNode = await loadCoValueOrFail(childAdmin.node, reader.accountID);
        const childGroupOnChildAdminNode = await loadCoValueOrFail(childAdmin.node, childGroup.id);
        await childGroupOnChildAdminNode.removeMember(readerOnChildAdminNode);
        expect(childGroupOnChildAdminNode.roleOf(reader.accountID)).toEqual(undefined);
    });
});
//# sourceMappingURL=group.removeMember.test.js.map