...
test("Admins an add writers to a group, who can't add admins, writers, or readers (high level)", () => {
    ...
    expect(() => groupAsWriter.addMember(otherAgent, "writer")).toThrow("Failed to set role due to insufficient permissions (role of current account is writer)");
...
test("Admins can add readers to a group, who can't add admins, writers, or readers (high level)", () => {
    ...
    expect(() => groupAsReader.addMember(otherAgent, "writer")).toThrow("Failed to set role due to insufficient permissions (role of current account is reader)");
    expect(() => groupAsReader.addMember(otherAgent, "reader")).toThrow("Failed to set role due to insufficient permissions (role of current account is reader)");
    expect(groupAsReader.get(otherAgent.id)).toBeUndefined();
});
