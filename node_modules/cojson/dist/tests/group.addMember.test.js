...
import { SyncMessagesLog, loadCoValueOrFail, setupTestAccount, setupTestNode, waitFor, } from "./testUtils.js";
beforeEach(async () => {
...
describe("Group.addMember", () => {
    test("an admin should be able to grant read access", async () => {
        ...
        await waitFor(() => {
            expect(expectMap(personOnReaderNode.core.getCurrentContent()).get("name")).toEqual("John Doe");
        });
    });
    test("an admin should be able to grant write access", async () => {
        ...
        await waitFor(() => {
            expect(expectMap(personOnWriterNode.core.getCurrentContent()).get("name")).toEqual("John Doe");
        });
    ...
    test("an admin should be able to grant writeOnly access", async () => {
        ...
        const writeOnlyUserOnAdminNode = await loadCoValueOrFail(admin.node, writeOnlyUser.accountID);
        group.addMember(writeOnlyUserOnAdminNode, "writeOnly");
        expect(group.roleOf(writeOnlyUser.accountID)).toEqual("writeOnly");
        const personOnWriteOnlyNode = await loadCoValueOrFail(writeOnlyUser.node, person.id);
    ...
    test("an admin should be able to grant admin access", async () => {
        ...
        const personOnReaderNode = await loadCoValueOrFail(reader.node, person.id);
        expect(personOnReaderNode.get("name")).toEqual(undefined);
        const readerOnOtherAdminNode = await loadCoValueOrFail(otherAdmin.node, reader.accountID);
    ...
    test("a reader should not be able to add a member", async () => {
        ...
        const readerOnAdminNode = await loadCoValueOrFail(admin.node, reader.accountID);
        group.addMember(readerOnAdminNode, "reader");
        const newUserOnReaderNode = await loadCoValueOrFail(reader.node, newUser.accountID);
    ...
    test("a writer should not be able to add a member", async () => {
        ...
        const writerOnAdminNode = await loadCoValueOrFail(admin.node, writer.accountID);
        group.addMember(writerOnAdminNode, "writer");
        const newUserOnWriterNode = await loadCoValueOrFail(writer.node, newUser.accountID);
    ...
    test("a writeOnly should not be able to add a member", async () => {
        ...
        const writeOnlyUserOnAdminNode = await loadCoValueOrFail(admin.node, writeOnlyUser.accountID);
        group.addMember(writeOnlyUserOnAdminNode, "writeOnly");
        const newUserOnWriteOnlyNode = await loadCoValueOrFail(writeOnlyUser.node, newUser.accountID);
        const groupOnWriteOnlyNode = await loadCoValueOrFail(writeOnlyUser.node, group.id);
...
//# sourceMappingURL=group.addMember.test.js.map