...
import { createAccountInNode, nodeWithRandomAgentAndSessionID, } from "./testUtils.js";
...
describe("getPriorityFromHeader", () => {
    ...
    test("returns MEDIUM priority for costream type", () => {
        ...
        expect(getPriorityFromHeader(costream.verified.header)).toEqual(CO_VALUE_PRIORITY.MEDIUM);
    });
    test("returns LOW priority for binary costream type", () => {
        ...
        expect(getPriorityFromHeader(costream.verified.header)).toEqual(CO_VALUE_PRIORITY.LOW);
    });
    test("returns HIGH priority for account type", async () => {
        const node = nodeWithRandomAgentAndSessionID();
        const account = createAccountInNode(node);
        expect(getPriorityFromHeader(account.account.core.verified.header)).toEqual(CO_VALUE_PRIORITY.HIGH);
    });
    test("returns HIGH priority for group type", () => {
        const node = nodeWithRandomAgentAndSessionID();
        const group = node.createGroup();
        expect(getPriorityFromHeader(group.core.verified.header)).toEqual(CO_VALUE_PRIORITY.HIGH);
    });
    test("returns MEDIUM priority for other types", () => {
        ...
        expect(getPriorityFromHeader(comap.verified.header)).toEqual(CO_VALUE_PRIORITY.MEDIUM);
        expect(getPriorityFromHeader(colist.verified.header)).toEqual(CO_VALUE_PRIORITY.MEDIUM);
    });
});
