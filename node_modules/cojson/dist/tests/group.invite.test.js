...
describe("Group invites", () => {
    test("should be able to accept a reader invite", async () => {
        ...
        const personOnNewMemberNode = await loadCoValueOrFail(newMember.node, person.id);
        expect(personOnNewMemberNode.get("name")).toEqual(undefined);
        await newMember.node.acceptInvite(group.id, inviteSecret);
        await waitFor(() => {
            expect(expectMap(personOnNewMemberNode.core.getCurrentContent()).get("name")).toEqual("John Doe");
        });
        const groupOnNewMemberNode = await loadCoValueOrFail(newMember.node, group.id);
    ...
    test("should be able to accept a writeOnly invite", async () => {
        ...
        await newMember.node.acceptInvite(group.id, inviteSecret);
        const groupOnNewMemberNode = await loadCoValueOrFail(newMember.node, group.id);
        expect(groupOnNewMemberNode.roleOf(newMember.accountID)).toEqual("writeOnly");
    ...
    test("should be able to accept an admin invite", async () => {
        ...
        const readerOnNewMemberNode = await loadCoValueOrFail(newMember.node, reader.accountID);
    ...
    test("invites should not downgrade the role of an existing member", async () => {
        ...
        const memberAccount = await loadCoValueOrFail(member.node, member.accountID);
    ...
    test("invites should be able to upgrade the role of an existing member", async () => {
        ...
        const readerAccount = await loadCoValueOrFail(member.node, reader.accountID);
...
//# sourceMappingURL=group.invite.test.js.map