...
}) & CoMap : S extends AnyCoListSchema<infer T> ? CoList<InstanceOrPrimitiveOfSchema<T>> : S extends AnyCoFeedSchema<infer T> ? CoFeed<InstanceOrPrimitiveOfSchema<T>> : S extends PlainTextSchema ? CoPlainText : S extends RichTextSchema ? CoRichText : S extends FileStreamSchema ? FileStream : S extends z.core.$ZodOptional<infer Inner> ? InstanceOrPrimitiveOfSchema<Inner> | undefined : S extends z.ZodJSONSchema ? JsonValue : S extends z.core.$ZodUnion<infer Members> ? InstanceOrPrimitiveOfSchema<Members[number]> : S extends z.core.$ZodObject<infer Shape> ? {
    -readonly [key in keyof Shape]: InstanceOrPrimitiveOfSchema<Shape[key]>;
...
} : S extends z.core.$ZodString ? string : S extends z.core.$ZodNumber ? number : S extends z.core.$ZodBoolean ? boolean : S extends z.core.$ZodLiteral<infer Literal> ? Literal : S extends z.core.$ZodDate ? Date : S extends z.core.$ZodEnum<infer Enum> ? Enum[keyof Enum] : S extends z.core.$ZodTemplateLiteral<infer pattern> ? pattern : S extends z.core.$ZodReadonly<infer Inner> ? InstanceOrPrimitiveOfSchema<Inner> : S extends z.core.$ZodDefault<infer Default> ? InstanceOrPrimitiveOfSchema<Default> : S extends z.core.$ZodCatch<infer Catch> ? InstanceOrPrimitiveOfSchema<Catch> : never : S extends CoValueClass ? InstanceType<S> : never;
